...
```
- 开启后，需要用到查询dept的时候才会调用相应的SQL语句![](./img/07.png)
...
- 二级缓存开启的条件
	...
	2. 在映射文件中设置标签<cache />
	3.一级缓存在没有提交或关闭sqlsession的时候，查询的数据会保存到一级缓存中
...
- 使二级缓存失效的情况：两次查询之间**执行了任意的增删改，会使一级和二级缓存同时失效**
...
- flushInterval属性：刷新间隔，单位毫秒
 ...
 - 默认情况是不设置，也就是没有刷新间隔，缓存仅仅调用语句（增删改）时刷新（指清空缓存）
...
逆向工程：是通过解析数据库中的表去反向生成当前的实体类、mapper接口以及mapper映射文件
...
- 如果出现报错：`Exception getting JDBC Driver`，可能是pom.xml中，数据库驱动配置错误
	- dependency中的驱动![](./img/15.png)
	- mybatis-generator-maven-plugin插件中的驱动![](./img/16.png)
	- 两者的驱动版本应该相同
- 执行结果![](./img/17.png)

## QBC：queryBy(根据条件查询)

**Criteria：条件的 意思**，在设置条件查询时可以嵌套使用

***逆向工程不支持多表联查，需要自己手动写SQL语句***
